function data_plotter(K,Llx,tf,dt,dts,Nens,sig,Nfloats)

KT = 2*K;

Xfloats = linspace(-Llx,Llx,Nfloats/2)';
[fin_dat_sfloat,tvals,msqerror_sfloat] = kalman_filter_experimenter(K,Llx,tf,dt,dts,Nens,sig,Xfloats);

Xfloats = linspace(-Llx,Llx,Nfloats)';
...
legend({'$\eta_{4f}(x,t_{f})$','$\eta_{8f}(x,t_{f})$','$\eta(x,t_{f})$'},'Interpreter','LaTeX')
...
legend({'$\eta_{4f}(x,t_{f})$','$\eta_{8f}(x,t_{f})$'},'Interpreter','LaTeX')
...
legend({'$\eta_{4f}(x,t)$','$\eta_{8f}(x,t)$'},'Interpreter','LaTeX')
